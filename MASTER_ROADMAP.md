...
## 42. M1-5 完了記録 ＋ M1 Must全完了（2026-06-30）

### M1-5 実装内容

`api/monitor-dashboard.ts`をM1-1の空データ応答から本格集計へ更新。

| 集計関数 | 対象 |
|---------|------|
| `aggregateContact` | `monitor:contact:runs:index` → Run → `:items` → ContactItem |
| `aggregateAppearance` | `monitor:appearance:runs:index` → Run → `:items` → MonitoringItem |
| `aggregateCrawl` | `monitor:crawl:logs:index` → CrawlLogEntry |

3関数は互いの出力を一切参照せず、完全に独立して実行・集計する（因果断定禁止の構造的徹底）。

**EntityDashboardRow**：`lastContactedAt` / `lastCrawledAt` / `lastAppearanceCheckedAt` / `contactCount` / `crawlCount` / `appearanceCount` / `appearanceRate` / `citationCount` / `byProvider`（Provider別に同5指標を独立集計）。

**period対応**：`7d` / `30d` / `all`。各指標は自分自身のタイムスタンプ（`requestedAt` / `checkedAt` / `detectedAt`）でフィルタする。

**禁止事項の遵守確認**：`causedBy` / `conversion` / `attribution` / `contactToAppearanceRate` / 合成スコアのいずれもコード中に存在しない（実装後にレビューで確認済み）。
...
|------|------|
| クリーンclone build | ✅ |
| 実データ集計（aisle Entity：contactCount=1, crawlCount=2, byProvider内訳） | ✅ |
| period=7d/30d/all フィルタ | ✅ |
| Entity別・Provider別集計 | ✅ |
| 因果断定フィールドなし | ✅ |
| 既存Studio/Authoring/Admin/Monitor M1-1〜M1-4無影響 | ✅（Authoring Engine APIチェーン27/27継続） |

### コミット

```
666e08c  feat: Aisle Monitor M1-5 — Dashboard real aggregation
```
...
| Must項目 | 状態 |
|---------|:----:|
| Manual AI Contact | ✅（M1-2） |
| Crawl Log | ✅（M1-4） |
| Appearance Monitoring（手動トリガー） | ✅（M1-3） |
| Dashboard最小版→本格集計 | ✅（M1-1→M1-5） |
| RefBase Entity/Reference一覧の読み取り連携 | ✅（M1-1） |

**M1 Must 5項目すべて完了。** API実装としてのM1スプリントはここで一区切り。Should（Provider別フィルタ/Entity別フィルタ/Crawl LogとContact Runの時間相関表示）はAPI側で既に対応済み（`?provider=`/`?entityId=`フィルタ、`relatedContactRuns`）。残るはUI実装（今回のM1スコープ外）。

### 次のアクション

→ M1 API群のUI実装、またはM1完了総括（テストデータ整理含む）へ進む。

---

## 43. M1完了総括（2026-06-30）

M1（Aisle Monitor MVP）の実装・検証・ドキュメント同期がすべて完了し、Phase 3 M1を正式に完了扱いとする。

### ① API一覧（全6本）

| API | メソッド | 責務 | KV書き込み | KV読み取り |
|-----|---------|------|-----------|-----------|
| `monitor-entities` | GET | RefBase Entity/Reference一覧の読み取り連携 | なし | `refbase:company:*` / `refbase:index:*` |
| `monitor-contact` | POST/GET | Manual AI Contact実行・履歴取得（perplexityのみ・simulated） | `monitor:contact:*` | `refbase:company:*` / `refbase:ref:*` |
| `monitor-appearance` | POST/GET | Appearance Monitoring実行・履歴取得（PERPLEXITY_API_KEY有無でreal/simulated切替） | `monitor:appearance:*` | `refbase:company:*` |
| `monitor-crawl-ingest` | POST | RefBase→Monitor Bot検知の受信（Fire-and-forget・`MONITOR_INGEST_SECRET`認証） | `monitor:crawl:*` | なし |
| `monitor-crawl-log` | GET | Crawl Log一覧・`relatedContactRuns`時間相関付与 | なし | `monitor:crawl:*` / `monitor:contact:run:*` |
| `monitor-dashboard` | GET | Contact/Crawl/Appearanceの独立集計（Entity別・Provider別・period別） | なし | `monitor:contact:*` / `monitor:appearance:*` / `monitor:crawl:*` |

全API共通：認証は`isAuthorized()`（`x-aisle-admin: 1` or `Bearer {EM_SHARED_SECRET}`）。例外は`monitor-crawl-ingest`のみ`MONITOR_INGEST_SECRET`専用方式（サーバー間連携のため分離）。

### ② Monitor KVキー一覧（全7プレフィックス）
...
RefBase既存KV（`refbase:company:*`等）は読み取り専用で参照するのみ。Monitor側からRefBase KVへの書き込みは一切なし。

### ③ テストデータ残存状況（2026-06-30 本番確認）

| 種別 | 件数 | 内容 |
|------|:---:|------|
| ContactRun | 1件 | `CR-aa1aedaf-...`（entity=aisle, provider=perplexity, simulated） |
| MonitoringRun | 1件 | `MR-2b5a2a65-...`（entity=aisle, provider=perplexity, simulated） |
| CrawlLogEntry | 2件 | `CL-886b1df9-...`（PerplexityBot）/ `CL-d08d5f29-...`（GPTBot）。いずれもentity=aisle |

**判定：実害なし。削除は任意。**
- 全件 `aisle`（Aisle自社）に対するテスト実行であり、クライアントEntityのデータではない
- 全件simulated（実Provider APIへの接触なし）または模擬Bot UA
- Dashboard集計（M1-5検証）はこのデータを根拠に正常動作を確認済み——消すとM1-5の検証証跡が失われる
- KV件数として無視できる規模（7キー）であり、本番運用上の負荷・コスト影響なし

**削除案（必要なら）**：UI実装時に「サンプルデータ」として活用し続けるか、UI実装直前に`kv.del`で7キーを個別削除するかの二択。今回は実装コードに触れない方針のため、削除コマンドの実行は行わない。**ユーザー判断待ち。**

### ④ 仕様書同期

- `MASTER_ROADMAP.md`：M1-1〜M1-5（Section 41-42）+ 本総括（Section 43）で完結
- `MONITOR_SPECIFICATION_V1.md`：「実装ゼロ」記述をM1完了内容に更新（本コミットで反映）
- `AISLE_PLATFORM_SPECIFICATION_V3.md`：Section 1.6 Monitor行の「未実装」表記を更新（本コミットで反映）
...
完了の定義：
- Must 5項目（Manual AI Contact / Crawl Log / Appearance Monitoring / Dashboard / RefBase読み取り連携）すべてAPI実装・本番デプロイ・実データ検証済み
- 因果断定ガードレール（causedBy/conversion/attribution禁止）を全API・全KVスキーマで遵守
- 既存Studio/Authoring/Admin機能への影響ゼロ（Authoring Engine APIチェーン27/27継続）
- ドキュメント（MASTER_ROADMAP / MONITOR_SPECIFICATION_V1 / AISLE_PLATFORM_SPECIFICATION_V3）が実装と同期

M1完了のブロッカーではない未実装項目：
- UI（管理画面）— M1はAPIのみが正式スコープ。Should/Could/Parking Lotの一部はAPI対応済みだがUIは0%
- Scheduled Contact（定期実行）— Parking Lot。現状は手動トリガーのみ
- RefBase側Bot検知ミドルウェアの実装 — `monitor-crawl-ingest`は受信側のみM1スコープ。送信側（RefBaseリポジトリ）の実装は別リポジトリ・別セッションの作業
- 実Provider接続の拡張（ChatGPT/Gemini Contact、Provider横断のAppearance） — 現状perplexityのみ

### ⑥ 次の進め方（ユーザー判断待ち）

| 選択肢 | 内容 |
|--------|------|
| A. UI実装 | `/monitor`管理画面（Dashboard表示・Contact実行ボタン・Crawl Log一覧・Appearance履歴） |
