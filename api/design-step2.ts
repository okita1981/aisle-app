...
// ── システムプロンプト（STEP4〜6 + サマリ） ──────────────────────────

const DESIGN_STEP2_SYSTEM_PROMPT = `あなたはAisle出現設計エンジン（後段評価）です。
STEP1〜3の設計結果（M-ID接点・SB-ID構文・After構文）を受け取り、
STEP4（構文接続順）・STEP5（勝因接続マトリクス）・STEP6（出現構造評価）・サマリを生成してください。
...
- step6は「出現構造評価マトリクス」です。各SB-IDを以下の3軸で評価してください（全SB-ID必須）。
  ...
  STEP4（勝因接続マトリクス）の分析結果を踏まえて、各SB-IDの改善レバーを具体化してください。
...
- 重要：comment・connectionComment・winningFactor・gapToAisle・requiredAction・improvementLever・overallImpression・keyBun・complementNeeds・implementationProposalなどクライアントに表示されるテキストフィールドには、ID記号（A-01、E-03、AISLE-01-01-A、M-07、K-01などの英数字コード）を一切含めないこと。IDはsbId・mId・tId・aId・kId・winningEId・mainKIdなどの専用フィールドにのみ記載する。数値目標（出現率○%向上など）も記載せず、定性的な表現のみ使用すること。
- 【JSON出力必須ルール・厳守】
